...
BIN      = $(GOPATH)/bin
BASE     = $(GOPATH)/src/github.com/llimon/churndr/$(PACKAGE)
FULLPATH = github.com/llimon/churndr/$(PACKAGE)
...
all: fmt lint vendor | $(BASE) ; $(info $(M) building executable…) @ ## Build program binary
	$Q cd $(BASE) && CGO_ENABLED=0  $(GO) build \
		...
		-a -o bin/churndr main.go

.PHONY: build-linux 
build-linux: $(BASE) ; $(info $(M) building executable…) @ ## Build program binary 
	$Q cd $(BASE) && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 $(GO) build \
		-tags release \
		-ldflags '-X $(PACKAGE)/cmd.Version=$(VERSION) -X $(PACKAGE)/cmd.BuildDate=$(DATE)' \
		-a -tags netgo -ldflags '-w' -o bin/churndr main.go


$(BASE): ; $(info $(M) setting GOPATH…)
	@mkdir -vp $(dir $@)
	#@mkdir -vp $(dir $@)/github.com/churndr
	#@ln -sf $(CURDIR) $(dir $@)/github.com/churndr
	@ln -sf $(CURDIR) $@
...
test-coverage: fmt lint vendor test-coverage-tools | $(BASE) ; $(info $(M) running coverage tests…) @ ## Run coverage tests
	...
	$Q curl -s https://codecov.tools.a.intuit.com/bash | bash -s - -t ${CODECOV_TOKEN}
...
swaggerserver: $(BASE) $(GOSWAGGER) ; $(info $(M) running swagger server…) @ ## Run golint
	$Q cd $(BASE) && ret=0 && for pkg in $(PKGS); do \
		test -z "$$($(GOSWAGGER) generate server -f ./swagger.json -A churndr --principal churndr  | tee /dev/stderr)" || ret=1 ; \
	 done ; exit $$ret
...
.PHONY: package 
package: build-linux server.crt ; $(info $(M) Creating creating docker image...) @ ## Run openssl
	@ret=0 && \
        docker build . -f ./Dockerfile -t churndr:latest || ret=$$?; \
        exit $$ret
...
run:  stop ; $(info $(M) Running docker container...) @ ## Run openssl
	@ret=0 && \
	docker run -d --rm -p 8443:8443 -v ~/.idps/:/.idps/ --name churndr churndr:latest || ret=$$?; \
        exit $$ret

.PHONY: logs
logs:  logs ; $(info $(M) Get docker logs for churndr image...) @ ## Run openssl
	@ret=0 && \
	docker logs churndr || ret=$$?; \
        exit $$ret
